...
Scene.prototype.changeDirection = function (event, dir) {
    ...
    if (this.snake.hasMove == true && direction[0] != -this.snake.direction[0] && direction[1] != -this.snake.direction[1]) {
...
Scene.prototype.moveSnake = function () {
    ...
    // 是否撞墙
    if (!this.setting.cross) {
        if (newPosition[0] < 0 || newPosition[0] >= this.setting.column || newPosition[1] < 0 || newPosition[1] >= this.setting.row) {
            alert('撞墙啦，你的分数：' + this.info.score + '。 小蛇长度：' + this.snake.position.length + '。');
            endGame();
        }
    } else {
        newPosition[0] = (newPosition[0] + this.setting.column) % this.setting.column;
        newPosition[1] = (newPosition[1] + this.setting.row) % this.setting.row;
    }

    // 是否撞到自己
    for (let i = 1; i < this.snake.position.length; i++) {
        if (newPosition[0] == this.snake.position[i][0] && newPosition[1] == this.snake.position[i][1]) {
            alert('撞到自己啦，你的分数：' + this.info.score + '。 小蛇长度：' + this.snake.position.length + '。');
            endGame();
        }
    }
}
